...
class PersonnagesManager
{
  ...
  {
    if (is_int($info)) {
      $q = $this->db->query('SELECT id, nom, degats, timeEndormi, type, atout, forcePerso, experience, niveau FROM personnages_v2 WHERE id = ' . $info);
      $perso = $q->fetch(PDO::FETCH_ASSOC);
    } else {
      $q = $this->db->prepare('SELECT id, nom, degats, timeEndormi, type, atout, forcePerso, experience, niveau FROM personnages_v2 WHERE nom = :nom');
      $q->execute([':nom' => $info]);
  ...
  public function getList($nom)
  {
    ...
    $q = $this->db->prepare('SELECT id, nom, degats, timeEndormi, type, atout, forcePerso, experience, niveau FROM personnages_v2 WHERE nom <> :nom ORDER BY nom');
    $q->execute([':nom' => $nom]);
  ...
  {
    $q = $this->db->prepare('UPDATE personnages_v2 SET degats = :degats, timeEndormi = :timeEndormi, atout = :atout, forcePerso = :forcePerso, experience = :experience, niveau = :niveau WHERE id = :id');
